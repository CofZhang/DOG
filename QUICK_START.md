...
| LED5 | PE1 | 电机1-3掉线 | 闪烁次数指示具体掉线电机（见13.6节） |
| LED6 | PE2 | 电机4-6掉线 | 闪烁次数指示具体掉线电机（见13.6节） |
| LED7 | PE3 | 电机7-9掉线 | 闪烁次数指示具体掉线电机（见13.6节） |
| LED8 | PE4 | 电机10-12掉线 | 闪烁次数指示具体掉线电机（见13.6节） |
...
| LED5~LED8任意闪烁 | 对应组有电机掉线，观察闪烁次数定位具体电机，同时蜂鸣器间歇报警 |
| LED5~LED8全部闪烁 | 所有电机掉线，检查CAN总线和电机供电 |
...
LED5~LED8 通过**闪烁次数**指示组内具体哪个电机掉线：

| 闪烁次数 | LED5 | LED6 | LED7 | LED8 |
|---------|------|------|------|------|
| 闪 **1** 次 | 电机1 | 电机4 | 电机7 | 电机10 |
| 闪 **2** 次 | 电机2 | 电机5 | 电机8 | 电机11 |
| 闪 **3** 次 | 电机3 | 电机6 | 电机9 | 电机12 |
...
同组两个电机掉线（例：电机4和电机6）：
  LED6  ██ ──500ms── ██ ██ ██ ─1s─ ██ ──500ms── ██ ██ ██ ─1s─ ...
        ↑闪1次(电机4)              ↑闪3次(电机6)

全组三个都掉线（例：电机7/8/9）：
  LED7  ██ ─500ms─ ██ ██ ─500ms─ ██ ██ ██ ─1s─ 重复
        ↑1次       ↑2次           ↑3次
```

**读灯方法：**
1. 观察哪个LED在闪烁 → 确定掉线在哪组（LED5=1-3组，LED6=4-6组，LED7=7-9组，LED8=10-12组）
2. 数清楚每段连续闪烁的次数 → 对照上表确定具体电机编号
3. 若同一个LED有多段不同次数的闪烁轮流出现 → 该组有多个电机同时掉线

**注意：** 掉线检测仅在系统收到第一包USB数据后（进入运行状态）才启用，上电未连接上位机时不会误报。
